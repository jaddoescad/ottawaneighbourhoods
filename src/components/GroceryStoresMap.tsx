...
interface GroceryStoresMapProps {
  boundaries: NeighbourhoodBoundary[];
  stores: FoodEstablishmentData[];
  neighbourhoodName: string;
}

export default function GroceryStoresMap({
  boundaries,
  stores,
  neighbourhoodName
}: GroceryStoresMapProps) {
  ...
  const center = useMemo(() => {
    ...
    if (minLat === Infinity && stores.length > 0) {
