...
export default function BestSchools() {
  ...
  // Filter out neighbourhoods with no schools, then sort by school score
  const sorted = [...neighbourhoods]
    .filter((n) => n.details.schools > 0)
    .sort((a, b) => (b.categoryScores?.schools || 0) - (a.categoryScores?.schools || 0));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-4 text-center">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
          Best Schools in Ottawa by Neighbourhood
        </h1>
        <p className="text-gray-600 mt-2 text-sm sm:text-base">
          Ranked by EQAO test scores and school availability
        </p>
      </div>

      <FilterBar activeFilter="schools" />
