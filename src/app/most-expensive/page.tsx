...
export default function MostExpensive() {
  ...
  const sorted = [...neighbourhoods].sort((a, b) => b.avgRent - a.avgRent);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-4 text-center">
        ...
        <p className="text-gray-600 mt-2 text-sm sm:text-base">
          Ranked by average rent prices (highest first)
        </p>
      </div>

      <FilterBar activeFilter="expensive" />

      <main className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
          {sorted.map((neighbourhood, index) => (
            <NeighbourhoodCard key={neighbourhood.id} neighbourhood={neighbourhood} rank={index + 1} metric="rent" />
          ))}
        </div>
      </main>
    </div>
