...
export default function BestTransit() {
  const [showCoverageMap, setShowCoverageMap] = useState(false);

  const sorted = [...neighbourhoods].sort((a, b) => b.transitScore - a.transitScore);
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-4 text-center">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
          Best Transit in Ottawa by Neighbourhood
        </h1>
        <p className="text-gray-600 mt-2 text-sm sm:text-base">
          Ranked by Transit Score - O-Train access and bus coverage
        </p>
      </div>

      <FilterBar activeFilter="transit" />

      <main className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
          {sorted.map((neighbourhood, index) => (
            <NeighbourhoodCard key={neighbourhood.id} neighbourhood={neighbourhood} rank={index + 1} metric="transit" />
          ))}
        </div>
      </main>
    </div>
