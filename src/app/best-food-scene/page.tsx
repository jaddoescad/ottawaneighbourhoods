...
export default function BestFoodScene() {
  ...
  const sorted = [...neighbourhoods].sort((a, b) =>
    (b.details.restaurants || 0) - (a.details.restaurants || 0)
  );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-4 text-center">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
          Best Food Scene in Ottawa by Neighbourhood
        </h1>
        <p className="text-gray-600 mt-2 text-sm sm:text-base">
          Ranked by number of restaurants and dining options
        </p>
      </div>

      <FilterBar activeFilter="food" />

      <main className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
          {sorted.map((neighbourhood, index) => (
            <NeighbourhoodCard key={neighbourhood.id} neighbourhood={neighbourhood} rank={index + 1} metric="food" />
          ))}
        </div>
      </main>
    </div>
