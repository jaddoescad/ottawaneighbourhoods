...
export default function BestElementarySchools() {
  const [showCoverageMap, setShowCoverageMap] = useState(false);

  // Filter to neighbourhoods with elementary schools that have EQAO scores, then sort
  const sorted = [...neighbourhoods]
    .filter((n) => n.details.elementaryWithEqaoScores > 0)
    .sort((a, b) => (b.details.avgEqaoScoreElementary || 0) - (a.details.avgEqaoScoreElementary || 0));
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-4 text-center">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
          Best Elementary Schools in Ottawa by Neighbourhood
        </h1>
        <p className="text-gray-600 mt-2 text-sm sm:text-base">
          Ranked by EQAO test scores for elementary schools (Grades 3 & 6)
        </p>
      </div>

      <FilterBar activeFilter="elementary-schools" />

      <main className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
          {sorted.map((neighbourhood, index) => (
            <NeighbourhoodCard key={neighbourhood.id} neighbourhood={neighbourhood} rank={index + 1} metric="elementary-schools" />
          ))}
        </div>
      </main>
    </div>
