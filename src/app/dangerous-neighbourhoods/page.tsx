...
export default function DangerousNeighbourhoods() {
  ...
  // Sort by Safety Score (lowest first = most dangerous)
  // Safety Score combines: Violent Crime (40%), Collisions (35%), Property Crime (15%), Overdose (10%)
  const sortedByDanger = [...neighbourhoods].sort((a, b) =>
    (a.categoryScores?.safety || 0) - (b.categoryScores?.safety || 0)
  );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-4 text-center">
        <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">
          Most Dangerous Neighbourhoods in Ottawa
        </h1>
        <p className="text-gray-600 mt-2 text-sm sm:text-base">
          Ranked by Safety Score: crime, collisions, and overdose data combined
        </p>
      </div>

      {/* Filter Buttons */}
      <FilterBar activeFilter="dangerous" />

      {/* Grid */}
      <main className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
          {sortedByDanger.map((neighbourhood, index) => (
            <NeighbourhoodCard
