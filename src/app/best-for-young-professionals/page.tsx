...
export default function BestForYoungProfessionals() {
  ...
  // Young professional score = transit + walkability + amenities + nightlife/food
  const sorted = [...neighbourhoods].sort((a, b) => {
    const ypScoreA = a.transitScore * 0.25 +
                     a.walkScore * 0.25 +
                     (a.categoryScores?.amenities || 0) * 0.25 +
                     Math.min(100, (a.details.restaurants || 0) * 2) * 0.25;
    const ypScoreB = b.transitScore * 0.25 +
                     b.walkScore * 0.25 +
                     (b.categoryScores?.amenities || 0) * 0.25 +
                     Math.min(100, (b.details.restaurants || 0) * 2) * 0.25;
    return ypScoreB - ypScoreA;
  });
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 pt-4 text-center">
        ...
        <p className="text-gray-600 mt-2 text-sm sm:text-base">
          Ranked by transit, walkability, amenities, and dining scene
        </p>
      </div>

      <FilterBar activeFilter="young-professionals" />

      <main className="max-w-7xl mx-auto px-4 py-4">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-4">
          {sorted.map((neighbourhood, index) => (
            <NeighbourhoodCard key={neighbourhood.id} neighbourhood={neighbourhood} rank={index + 1} metric="walk" />
          ))}
        </div>
      </main>
    </div>
